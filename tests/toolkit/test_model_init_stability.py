...
def test_ttm_from_pretrained():
    ...
    m_4 = load_file(path / "init_stability_test/no_post_init" / "model.safetensors")

    assert_models_equal(m, m_4, "no post init")

    m_post_init = ttm_dummy_model({"post_init": True})
    m_4_post_init = load_file(path / "init_stability_test/post_init" / "model.safetensors")
    ...
    # m = ttm_dummy_model({"post_init": False})
    # m.save_pretrained("init_stability_test/no_post_init")
    # m_with_init = ttm_dummy_model({"post_init": True})
    # m_with_init.save_pretrained("init_stability_test/post_init")

    # with pytest.raises(AssertionError):
    #     assert_models_equal(m, m_with_init)
