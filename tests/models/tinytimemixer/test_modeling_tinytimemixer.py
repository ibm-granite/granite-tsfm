...
class TinyTimeMixerFunctionalTests(unittest.TestCase):
    ...
    def check_module(
        ...
        if task == "forecast":
            ...
            if config.multi_quantile_heads is not None:
                target_output = target_output.unsqueeze(1).expand(-1, len(config.multi_quantile_heads), -1, -1)
    ...
    def test_forecast_quantile(self):
        params = self.__class__.params.copy()
        params.update(multi_quantile_heads=[0.2, 0.5, 0.8], multi_quantile_head_weights=[1, 2, 1])

        self.check_module(task="forecast", params=params, output_hidden_states=True)
        # self.forecast_full_module(self.__class__.params, output_hidden_states = True)
