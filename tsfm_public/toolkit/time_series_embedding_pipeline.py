...
#
"""Hugging Face Pipeline for Time Series Embedding Tasks"""
...
# MODEL_FOR_TIME_SERIES_EMBEDDING_MAPPING_NAMES = OrderedDict(
#     [
#         # Model for Time Series embedding
...
class TimeSeriesEmbeddingPipeline(TimeSeriesPipeline):
    """
    Time Series Embedding using HF time series encoder models. This pipeline consumes a `pandas.DataFrame`
    containing the time series data and produces a new `pandas.DataFrame` containing the Embedding results.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs["full_series"] = full_series
        ...
        self._model_output_key = "decoder_hidden_state"  # TODO: check
        self._copy_dataset_keys = False

        if self.framework == "tf":
            raise ValueError(f"The {self.__class__} is only available in PyTorch.")

        # self.check_model_type(MODEL_FOR_TIME_SERIES_EMBEDDING_MAPPING)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Main method of the embedding pipeline. Takes the input time series data (in tabular format) and
    ...
    def postprocess(self, input, **kwargs):
        ...
        input = torch.cat(input, dim=0).detach().cpu().numpy()

        # embeddings
        num_reg_tokens = self.model.config.patch_register_tokens
        d_model = self.model.config.decoder_d_model_layerwise[-1]
        reg_emb_size = num_reg_tokens * d_model
        out["embedding"] = input[:, :, -reg_emb_size:].tolist()
