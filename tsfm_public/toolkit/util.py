...
def convert_tsfile_to_dataframe(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        ...
        for line in file:
            if line.startswith("#"):
                ...
                if timestamp_line[-1].strip()=='true':
                    timestamps = True

            elif line.startswith("@seriesLength") or line.startswith("@serieslength"):
                seriesLength_line = line.split()
                seriesLength = int(seriesLength_line[-1].strip())
                ...
                if not timestamps:
                    ...
                    split_line = [element.split(",") for element in split_line]

                    sub_df['id'] = [id]*seriesLength

                    for i, column in enumerate(split_line[:-1]):
                        sub_df[f'value_{i}'] = column
                    ...
                    sub_df.insert(0, "timestamp", timestamp_column)

                target = split_line[-1][0].strip()
                sub_df['target'] = [target]*seriesLength
