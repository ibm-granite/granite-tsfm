...
# Paper: https://www.arxiv.org/abs/2508.05287
#
...
class FlowStateCausalRevIN(nn.Module):
    ...
    def _get_statistics(self, x):
        ...
        mask = 1. if not self.missing else 1 - x[:,:,1:]
    ...
    if degree > 0:
        ...
        for ii in range(1, degree):
            retvar[:, ii+1] = ((2 * ii + 1) * x * retvar[:, ii] - \
                               ii * retvar[:, ii-1]) / (ii + 1)
    if retvar.ndim == 3: retvar = retvar.permute(2,0,1)
    return retvar
...
class FlowStateEncoderOutput(ModelOutput):
    ...
    """
    # TODO boh: Names of the attributes should be similar to the prior models
...
class FlowStateS5Block(nn.Module):
    ...
    def init_params(self):    
        with torch.no_grad():
            ...
            if state_dim % encoder_num_hippo_blocks != 0:
                raise ValueError(f'encoder_state_dim has to be divisible by encoder_num_hippo_blocks.')
            block_size = int(state_dim / encoder_num_hippo_blocks)
            # blockwise Hippo-N diagonalized
            n = torch.sqrt(2*(torch.arange(block_size) + 1.) + 1)
            A = -torch.outer(n, n)/2
            ...
            B = 0.5/torch.sqrt(torch.tensor(H))*(torch.randn(state_dim, H) + torch.randn(state_dim, H)* 1.j)
            C = 0.5/torch.sqrt(torch.tensor(state_dim))*(torch.randn(H, state_dim) + torch.randn(H, state_dim)* 1.j)
    ...
    def get_discretized(self, L=None, scale_factor=1.0):
        ...
        B_tilde = self.B_tilde_r + 1.j * self.B_tilde_i
        if type(scale_factor) is torch.Tensor and scale_factor.ndim > 0: # different scale factor per sample
            ...
            B_bar = (1 / lambda_ * (kernel[:,-2] - 1.))[..., None] * B_tilde
        else:
            log_Lambda_bar = scale_factor * lambda_ *torch.exp(self.log_Delta)
            kernel =  torch.multiply(log_Lambda_bar.unsqueeze(0), torch.arange(L-1, -1, -1, device=device).unsqueeze(-1)).exp()
    ...
    def forward(self, input_sequences, scale_factor=1.0):
        ...
        input_sequences = input_sequences + 0.j
        ...
        xs = torch.einsum('hn,...bn->...bh', self.C_tilde_r + 1.j * self.C_tilde_i, xs)
...
class FlowStateS5Layer(nn.Module):
    ...
    def forward(self, x, scale_factor=1.0):
        skip = x if (not self.last or x.ndim == 2) else x[min(self.config.min_context, x.shape[0])-1:] # last layer doesn't need MLP on all timesteps
        # SSM
...
class FlowStateEncoder(FlowStatePreTrainedModel):
    ...
    def __init__(self, config: FlowStateConfig):
        ...
        self.layers = nn.ModuleList([FlowStateS5Layer(config, last=(i==config.encoder_num_layers-1), ssm=True) for i in range(config.encoder_num_layers)])
    ...
    def forward(
        ...
        return FlowStateEncoderOutput(last_hidden_state=output,
...
class FlowStateFunctionalBasisDecoder(FlowStatePreTrainedModel):
    ...
    def forward(self, 
        ...
        It first linearly encodes the `encoder_output`, then samples the continuous basis functions based on `prediction_length` and `scale_factor` producing a set of configured discrete basis functions and 
        finally applies the linearly encoded `encoder_output` to the discrete basis function by matrix multiplication to produce the (time-)scaled prediction for the Forecasting Horizon.
        ...
        if W.ndim == 2:
            return FlowStateDecoderOutput(last_hidden_state=torch.einsum('...h,ph->...p', hidden_state, W).unsqueeze(-1),
                                          hidden_states=[hidden_state])
        else:
            return FlowStateDecoderOutput(last_hidden_state=torch.einsum('...bqh,bph->...bqp', hidden_state, W).unsqueeze(-1),
                                          hidden_states=[hidden_state])
...
class FlowStateModel(FlowStatePreTrainedModel):
    def __init__(self, config: FlowStateConfig):
        ...
        logger.info(f'Number of dencoder parameters: {decoder_paras*1e-3}k ({100*decoder_paras/trainable_paras:.2f}%)')
    ...
    def forward(
        ...
        if self.config.with_missing:
            ...
            time_first_past_values = torch.nan_to_num(time_first_past_values, 0.)
            time_first_past_values = torch.cat((time_first_past_values, mask), dim=-1)
            if mask_n > 0:
                apdx = torch.cat((torch.zeros(mask_n, batch, n_ch), torch.ones(mask_n, batch, 1)), dim=-1).to(past_values.device)
...
class FlowStateForPrediction(FlowStatePreTrainedModel):
    ...
    def forward(
        ...
        # TODO boh: Review the parameters, especially introduce the future_values because
        # one might need them in the future for fine-tuning
        r"""
        past_values (`torch.FloatTensor` of shape `(batch_size, seq_length, num_input_channels)`):
            Context values of the time series. For a forecasting task, this denotes the history/past time series values.
            For univariate time series, `num_input_channels` dimension should be 1. For multivariate time series, it is
        ...
        if batch_first:
            ...
            past_values = past_values[-max_context:]
        if mask_n > 0:
            self.model.config.min_context = past_values.shape[1] if batch_first else past_values.shape[0] # min context from which to start predicting
        else:
        ...
        model_output.last_hidden_state = self._combine_cpm_predictions(model_output.last_hidden_state, prediction_length)
