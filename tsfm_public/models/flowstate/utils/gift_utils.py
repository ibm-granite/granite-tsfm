...
def get_fixed_factor(freq: str, domain=None):
    ...
    if freq == '4S': ############### sub any reasonable cycle group --> Hourly
        ...
        factor = BASE_SEASON / 4 # only CMIP6 in pretraining --> 24. / 4 or 24 / 365*4 would be better! (TODO?)
    elif freq == 'D':
        if has_weekly: #  TODO: hurts performance sometimes (when? --> hlegs check synthetic data)
