...
# from gift_utils import get_fixed_factor
from tsfm_public.models.flowstate.utils.gift_utils import get_fixed_factor


class FlowState_Gift_Wrapper:
    def __init__(self, model, prediction_length, batch_size=64, n_ch=1, f='h', device='cpu', domain=None, no_daily=False):
        self.model = model
    ...
    def fill_nan(self, seq, min_len=10):
        ...
        # ensure min length (TODO: necessary?)
        if len(seq) < min_len:
    ...
    def get_batched(self, test_data):
        ...
        for idx, seq in enumerate(test_data):
            if seq['target'].ndim == 1:
                ...
                # TODO would throw error if not same number of nan per channel
    ...
    def predict(self, test_data):
        ...
        for idx, batch in enumerate(batched_data):
            pred = self.model(past_values=batch.to(self.device),
                              scale_factor=self.scale_factor,
                              prediction_length=self.prediction_length,
                              batch_first=False).prediction_outputs
        ...
        for i, i_ in enumerate(indeces): preds_[i_] = preds[i]
        return preds_
