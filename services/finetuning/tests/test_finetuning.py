...
def get_finetuning_response(
    msg: Dict[str, Any],
) -> pd.DataFrame:
    URL = "http://127.0.0.1:8000/v1/finetuning/forecasting"
    headers = {}
...
def test_zero_shot_forecast_finetuning(ts_data):
    ...
    df_out = get_finetuning_response(msg)
    assert len(df_out) == 1
...
def test_trained_model_finetuning(ts_data, model_path):
    test_data, params = ts_data
    id_columns = params["id_columns"]
