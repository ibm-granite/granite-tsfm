...
def granite_tsfm_forecaster(df,timestamp_column,target_columns,model_name = 'ttm',model_checkpoint=None,context_length=50,prediction_length=15,fixed_context = False):
    ...
    forecast = fpipe(df)
    y_pred = np.array([np.stack(z) for z in forecast[[f"{target_column}_prediction" for target_column in target_columns]].values]).transpose(0, 2, 1)
    y_true = np.array([np.stack(z) for z in forecast[[target_column for target_column in target_columns]].values]).transpose(0, 2, 1)
