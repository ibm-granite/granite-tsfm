...
dataset_properties_map = json.load(open("../gift_leaderboard_ttm_r3_nc/resources/dataset_properties.json"))
...
# ============================================================================
# Ray Distributed Computing Initialization
# ============================================================================
# Initialize Ray for parallel processing across multiple GPUs
# TODO: Make temp directory configurable for cross-platform compatibility
try:
    ray.init(ignore_reinit_error=True, _temp_dir="/tmp")
except ConnectionError:
    print("No runnning Ray instance found, creating a local instance...")
    ray.init(address=None, ignore_reinit_error=True, _temp_dir="/tmp")


# ============================================================================
# CSV Writer Actor for Thread-Safe File Writing
# ============================================================================
# Ray actor to handle concurrent CSV writes from multiple processes
@ray.remote
class CSVWriter:
...
def run_dataset(ds_name, writer_actor, done_datasets):
    ...
    for term in terms:
        ...
        # writer_actor.write_row.remote(result)
        ray.get(writer_actor.write_row.remote(result))
