...
def get_freq_mapping():
    ...
    freq_token_mapping["M"] = torch.Tensor([11])
    freq_token_mapping["A-DEC"] = torch.Tensor([11])
    freq_token_mapping["Q-DEC"] = torch.Tensor([11])

    # Seconds are currently mapped to OOV
    freq_token_mapping["10S"] = torch.Tensor([10])
        ...
        for i, _ in enumerate(self.X):
            ...
            if self.X[i]["target"].shape[1] < self.seq_len + self.forecast_len:
                ...
                if (
                    ...
                    >= (min_context_needed_mult + 1) * self.forecast_len
                ):
    ...
    def __getitem__(self, idx):
        if self.last_window_only:
            seq_x = self.X[idx]["target"][
                :, -(self.seq_len + self.forecast_len) : -self.forecast_len
            ]
