...
class TSAD_Pipeline(BaseDetector):
    def __init__(
        ...
        **kwargs
        ...
        self._model = TSPulseForReconstruction.from_pretrained(MODEL_PATH,
                                                               num_input_channels=num_input_channels,
                                                               scaling="revin",
                                                               mask_type="user")
        prediction_mode_array = [s_.strip() for s_ in str(prediction_mode).split('+')]
        self._scorer = TimeSeriesAnomalyDetectionPipeline(self._model,
                                                          ...
                                                          least_significant_scale=0.,
                                                          least_significant_score=1.)
    ...
    def decision_function(self, X):
        ...
        if not isinstance(score, pd.DataFrame) or ('anomaly_score' not in score):
            raise ValueError(f"Error: expect anomaly_score column in the output!")
