...
class ForecastDataset(torch.utils.data.Dataset):
    def __init__(self, data, window_size, pred_len, stride=1, normalize=True):
        ...
        self.univariate = self.data.shape[1] == 1
        self.sample_num = max(
            (self.data.shape[0] - window_size - pred_len) // stride + 1, 0
        )
    ...
    def _generate_samples(self):
        ...
        Y = torch.stack(
            [
                data[i + self.window_size : i + self.window_size + self.pred_len]
                for i in indices
            ]
        )
...
class ReconstructDataset_TSPulse(torch.utils.data.Dataset):
    def __init__(
        self,
        data,
        window_size,
...
# forecasting task.
class TSPulseDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        data,
        window_size,
