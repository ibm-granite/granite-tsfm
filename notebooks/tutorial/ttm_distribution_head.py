...
def fewshot_finetune_eval(
    ...
    signature_columns = ["past_values", "future_values"]
    ...
    finetune_forecast_trainer.model.num_parallel_samples = 100

    forecast_list = []
    ground_truth_list = []
    for batch in test_dataloader:
        forecasts = finetune_forecast_trainer.model.generate(batch["past_values"])
        forecast_list.append(forecasts.sequences.numpy())
        ground_truth_list.append(batch["future_values"].numpy())

    forecasts_np = np.concatenate(forecast_list, axis=0)
    ground_truth_np = np.concatenate(ground_truth_list, axis=0)

    forecast_mean_np = np.mean(forecasts_np, axis=1)

    print(forecasts_np.shape, ground_truth_np.shape, forecast_mean_np.shape)

    print(np.mean((ground_truth_np - forecast_mean_np) ** 2))
...
finetune_forecast_trainer = fewshot_finetune_eval(
